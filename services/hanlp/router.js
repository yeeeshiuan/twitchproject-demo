...
const auth = require('express-austack')
const router = express.Router();
const debug = require('debug')('hanlp-api:router');
...
if (process.env['AUSTACK_CLIENT_ID'] && process.env['AUSTACK_CLIENT_SECRET']) {
	let CLIENT_ID = process.env['AUSTACK_CLIENT_ID'];
	let CLIENT_SECRET = process.env['AUSTACK_CLIENT_SECRET'];
	debug('Enable auth with Austack ClientId: %s', CLIENT_ID)
	router.use(auth(CLIENT_ID, CLIENT_SECRET));
}
